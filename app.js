(() => {
  ...
  function renderSkyRegions() {
    const state = states.skyregions || (states.skyregions = { loaded: false, loading: false, error: '', fov: 140, message: '', selected: '', showBoundaries: true, orient: null });
    app.innerHTML = `<h2>Constellation Regions</h2><div class="sky-layout"><section class="panel sky-panel"><canvas id="regionCanvas" width="1000" height="700" tabindex="0" aria-label="constellation region sphere"></canvas></section><aside class="panel"><label>FOV degrees<input id="regionFov" type="number" min="20" max="190" step="5" value="${state.fov}"></label><label><input id="regionBounds" type="checkbox" ${state.showBoundaries !== false ? "checked" : ""}> boundaries</label><div class="sky-nav-grid" aria-label="region movement controls"><button type="button" data-rmove="-1,-1">↖</button><button type="button" data-rmove="0,-1">↑</button><button type="button" data-rmove="1,-1">↗</button><button type="button" data-rmove="-1,0">←</button><button type="button" id="regionReset">○</button><button type="button" data-rmove="1,0">→</button><button type="button" data-rmove="-1,1">↙</button><button type="button" data-rmove="0,1">↓</button><button type="button" data-rmove="1,1">↘</button></div><div id="regionMsg" class="message">${esc(state.message || '')}</div></aside></div>`;
    ...
    function draw() {
      ...
      for (let gy = 0; gy < gh; gy++) {
        for (let gx = 0; gx < gw; gx++) {
          ...
          if (!v) continue;
          const name = nearestRegion(v, list);
          names[gy][gx] = name;
    ...
    function finishPointer(e) {
      ...
      if (lastPoint && totalDrag < 6) {
        ...
        if (v) {
          state.selected = nearestRegion(v);
    ...
    if (!state.loaded && !state.loading) {
      state.loading = true;
      loadSkyData().then(() => { state.loaded = true; state.loading = false; draw(); canvas.focus(); }).catch(err => { state.error = err.message; state.loading = false; draw(); });
    }
