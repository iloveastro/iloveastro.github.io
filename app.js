(() => {
  ...
  function renderSkyGuessr() {
    ...
    function draw() {
      ...
      // The plot is intentionally sparse: bright asterism stars, then just a small amount of extra context.
    ...
    function move(dx, dy, multiplier = 1) {
      const scale = state.fov / Math.min(canvas.width, canvas.height) * multiplier;
    ...
    function moveButton(x, y) {
      const px = Math.min(canvas.width, canvas.height) * 0.16;
      move(x * px, y * px, 1);
      canvas.focus();
    }
    ...
    $('#skyFov').addEventListener('input', e => { state.fov = Math.max(20, Math.min(190, parseFloat(e.target.value) || 100)); draw(); });
    ...
    canvas.addEventListener('pointerdown', e => { dragging = true; last = { x: e.clientX, y: e.clientY }; canvas.setPointerCapture(e.pointerId); canvas.focus(); });
