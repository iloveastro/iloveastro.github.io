(() => {
  ...
  async function loadConstellationBounds() {
    ...
    skyBoundsPromise = fetch(CONSTELLATION_BOUNDS_URL, { cache: 'force-cache' })
      .then(res => { if (!res.ok) throw new Error('Could not load official boundary GeoJSON.'); return res.json(); })
      .then(geo => {
  ...
  function renderSkyGuessr() {
    ...
    $('#skyAutoMag').addEventListener('change', e => { state.autoMag = e.target.checked; $('#skyMag').disabled = state.autoMag !== false; syncMagInput(); draw(); });
    $('#skyMag').addEventListener('input', e => { state.magLimit = Math.max(4, Math.min(6, parseFloat(e.target.value) || 4.8)); draw(); });
  ...
  function renderAlphaPin() {
    ...
    function ensureTarget() {
      if (!state.target && skyStars.length) {
        ...
        state.result = '';
        if (state.target) state.orient = makeBasisFromForward(state.target.star.v);
      }
    }
    ...
    function draw() {
      ...
      ctx.strokeStyle = 'black'; ctx.lineWidth = 1; ctx.strokeRect(0.5, 0.5, canvas.width - 1, canvas.height - 1);
      if (!state.loaded) {
        ctx.fillStyle = 'black'; ctx.font = '20px Arial'; ctx.fillText(state.error || 'loading alpha-pin sky...', 24, 40); return;
      }
      const radius = Math.min(canvas.width, canvas.height) * 0.48;
      const fovRad = state.fov * Math.PI / 180;
      const b = ensureOrientation();
    ...
    function newTarget() {
      ...
      state.submitted = false;
      state.result = '';
      state.orient = makeBasisFromForward(state.target.star.v);
      renderAlphaPin();
    }
    function submitGuess() {
      if (!state.target || !state.selectedVec || state.submitted) return;
      const angle = angularDeg(state.selectedVec, state.target.star.v);
      const score = Math.max(0, Math.min(5000, Math.round(5000 * (1 - angle / 180))));
      state.submitted = true;
      record('alphapin', score >= 4000);
      state.result = `${score}/5000 — ${angle.toFixed(1)}° away. Target: ${state.target.label}${state.target.fallback ? ' (fallback target)' : ''}`;
    ...
    $('#alphaCentre').addEventListener('click', () => { if (state.target) state.orient = makeBasisFromForward(state.target.star.v); draw(); canvas.focus(); });
    ...
    if (!state.loaded && !state.loading) {
      state.loading = true;
      loadSkyData().then(() => { state.loaded = true; state.loading = false; skyAlphaCache = null; ensureTarget(); draw(); canvas.focus(); }).catch(err => { state.error = err.message; state.loading = false; draw(); });
    }
  ...
  function renderSkyRegions() {
    ...
    app.innerHTML = `<h2>Constellation Regions</h2><div class="sky-layout"><section class="panel sky-panel"><canvas id="regionCanvas" width="1000" height="700" tabindex="0" aria-label="constellation region sphere"></canvas></section><aside class="panel"><label>FOV degrees<input id="regionFov" type="number" min="20" max="190" step="5" value="${state.fov}"></label><label><input id="regionBounds" type="checkbox" ${state.showBoundaries !== false ? "checked" : ""}> boundaries</label><label><input id="regionStars" type="checkbox" ${state.showStars !== false ? "checked" : ""}> stars</label><label>Star density / faintest magnitude<input id="regionMag" type="number" min="4" max="6" step="0.1" value="${state.magLimit}"></label><div class="sky-nav-grid" aria-label="region movement controls"><button type="button" data-rmove="-1,-1">↖</button><button type="button" data-rmove="0,-1">↑</button><button type="button" data-rmove="1,-1">↗</button><button type="button" data-rmove="-1,0">←</button><button type="button" id="regionReset">○</button><button type="button" data-rmove="1,0">→</button><button type="button" data-rmove="-1,1">↙</button><button type="button" data-rmove="0,1">↓</button><button type="button" data-rmove="1,1">↘</button></div><div class="controls"><button type="button" id="regionRollCCW">↺ rotate</button><button type="button" id="regionRollCW">rotate ↻</button><button type="button" id="regionClear">deselect</button></div><div id="regionMsg" class="message">${state.message || ''} </div></aside></div>`;
    ...
    function regionMessage(name, clickedVec, approximate = false) {
      ...
      const info = DATA.constellationInfo && DATA.constellationInfo[name];
      const meaning = info && info.meaning ? `<br><span class="small">${esc(info.meaning)}</span>` : '';
      const centreText = centre ? `<br>region centre: RA ${formatRA(centre.ra)}, Dec ${formatDec(centre.dec)}` : '';
      return `<strong>${esc(name)}${approximate ? ' (approximate)' : ''}</strong>${meaning}<br>clicked point: RA ${formatRA(clicked.ra)}, Dec ${formatDec(clicked.dec)}${centreText}`;
    }
    function vecAtCanvasPoint(x, y) {
      const b = ensureOrientation();
      const radius = Math.min(canvas.width, canvas.height) * 0.48;
    ...
    function draw() {
      ...
      ctx.strokeStyle = 'black'; ctx.lineWidth = 1; ctx.strokeRect(0.5, 0.5, canvas.width - 1, canvas.height - 1);
      if (!state.loaded) {
        ctx.fillStyle = 'black'; ctx.font = '20px Arial'; ctx.fillText(state.error || 'loading region sphere...', 24, 40); return;
      }
      const list = centres();
      const radius = Math.min(canvas.width, canvas.height) * 0.48;
      const cx = canvas.width / 2, cy = canvas.height / 2;
    ...
    function finishPointer(e) {
      ...
      if (lastPoint && totalDrag < 6) {
        ...
        if (v) {
          ...
          state.message = regionMessage(state.selected, v, !official);
    ...
    if (!state.loaded && !state.loading) {
      state.loading = true;
      Promise.all([loadSkyData(), loadConstellationBounds().catch(() => [])]).then(() => { state.loaded = true; state.loading = false; draw(); canvas.focus(); }).catch(err => { state.error = err.message; state.loading = false; draw(); });
    }
