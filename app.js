(() => {
  ...
  function renderSkyGuessr() {
    const state = states.skyguessr || (states.skyguessr = { loaded: false, loading: false, error: '', viewRa: 0, viewDec: 0, fov: 120, magLimit: 6.5, target: null, answered: false, message: '', score: scoreKey('skyguessr'), joystick: null });
    app.innerHTML = `<h2>SkyGuessr</h2><div class="sky-layout"><section class="panel sky-panel"><canvas id="skyCanvas" width="1000" height="700" tabindex="0" aria-label="celestial sphere"></canvas><div class="sky-joy" id="skyJoy"><div></div></div></section><aside class="panel"><label>FOV degrees<input id="skyFov" type="number" min="20" max="120" step="5" value="${state.fov}"></label><label>Magnitude limit<input id="skyMag" type="number" min="2" max="6.5" step="0.1" value="${state.magLimit}"></label><input id="skyAnswer" autocomplete="off" placeholder="constellation at the X"><div class="controls"><button type="button" id="skyNew">new location</button><button type="button" id="skyReveal">reveal</button></div><div id="skyMsg" class="message">${esc(state.message || '')}</div><div class="stats">${formatScore('skyguessr')}</div><p class="small">Drag, arrow keys, WASD, or the joystick. X marks the target coordinate. No constellation lines are drawn.</p></aside></div>`;
    const canvas = $('#skyCanvas'), ctx = canvas.getContext('2d');
    const answer = $('#skyAnswer');
    function ensureTarget() {
      if (!state.target && skyStars.length) {
        const candidates = skyStars.filter(s => s.mag <= 5.7 && s.constellation);
        state.target = rand(candidates);
    ...
    function project(v, b, radius, fovRad) {
      ...
      const rr = Math.tan(ang) / Math.tan(fovRad / 2) * radius;
    ...
    function draw() {
      ...
      if (!state.loaded) {
        ctx.fillStyle = 'black'; ctx.font = '20px Arial'; ctx.fillText(state.error || 'loading visible-star catalogue...', 24, 40); return;
      }
      ...
      for (const s of visible) {
        ...
        if (!p) continue;
        const r = Math.max(0.75, Math.min(4.2, 3.9 - s.mag * 0.48));
      ...
      if (state.target) {
        ...
        if (p) {
          ...
          ctx.beginPath(); ctx.moveTo(p.x - 12, p.y - 12); ctx.lineTo(p.x + 12, p.y + 12); ctx.moveTo(p.x + 12, p.y - 12); ctx.lineTo(p.x - 12, p.y + 12); ctx.stroke();
          ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(p.x, p.y, 18, 0, Math.PI * 2); ctx.stroke();
        }
      }
      ctx.restore();
    ...
    function move(dx, dy) {
      const scale = state.fov / Math.min(canvas.width, canvas.height);
      state.viewRa = (state.viewRa - dx * scale + 360) % 360;
      state.viewDec = Math.max(-88, Math.min(88, state.viewDec + dy * scale));
      draw();
    }
    ...
    function newTarget() {
      ...
      state.target = rand(candidates);
      const rd = raDecFromVec(state.target.v);
      state.viewRa = rd.ra + (Math.random() - 0.5) * Math.min(20, state.fov * 0.25);
      state.viewDec = Math.max(-85, Math.min(85, rd.dec + (Math.random() - 0.5) * Math.min(14, state.fov * 0.18)));
    ...
    $('#skyFov').addEventListener('input', e => { state.fov = Math.max(20, Math.min(120, parseFloat(e.target.value) || 120)); draw(); });
    $('#skyMag').addEventListener('input', e => { state.magLimit = Math.max(2, Math.min(6.5, parseFloat(e.target.value) || 6.5)); draw(); });
    ...
    canvas.addEventListener('pointermove', e => { if (!dragging) return; move(e.clientX - last.x, e.clientY - last.y); last = { x: e.clientX, y: e.clientY }; });
    canvas.addEventListener('pointerup', () => { dragging = false; });
    canvas.addEventListener('keydown', e => {
      const step = e.shiftKey ? 65 : 25;
    ...
    const joy = $('#skyJoy'), knob = joy.querySelector('div');
    let joyTimer = null;
    function stopJoy() { clearInterval(joyTimer); joyTimer = null; knob.style.transform = 'translate(-50%, -50%)'; }
    joy.addEventListener('pointerdown', e => {
      joy.setPointerCapture(e.pointerId);
      const rect = joy.getBoundingClientRect();
      const update = ev => {
        const x = ev.clientX - rect.left - rect.width / 2, y = ev.clientY - rect.top - rect.height / 2;
        const m = Math.hypot(x, y), lim = rect.width * 0.32, k = m > lim ? lim / m : 1;
        const jx = x * k, jy = y * k;
        knob.style.transform = `translate(calc(-50% + ${jx}px), calc(-50% + ${jy}px))`;
        clearInterval(joyTimer); joyTimer = setInterval(() => move(jx * 0.9, jy * 0.9), 40);
      };
      update(e);
      joy.onpointermove = update;
    });
