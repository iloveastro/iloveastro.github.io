(() => {
  ...
  function renderAlphaPin() {
    ...
    $('#alphaCentre').addEventListener('click', () => { randomOrientation(); draw(); canvas.focus(); });
    ...
    if (!state.loaded && !state.loading) {
      state.loading = true;
      loadSkyData().then(() => { state.loaded = true; state.loading = false; skyAlphaCache = null; ensureTarget(); draw(); canvas.focus(); }).catch(err => { state.error = 'sky data unavailable'; state.loading = false; draw(); });
    }
